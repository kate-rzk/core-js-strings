...
function removeFirstOccurrences(str, value) {
  ...
  const suffix = str.substring(index + value.length);
  const newStr = prefix.concat(suffix);
  return newStr;
}
...
function removeLastOccurrences(str, value) {
  ...
  const suffix = str.substring(lastIndex + value.length);
  const newStr = prefix.concat(suffix);
  return newStr;
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, function rot(char) {
    const startCode = char <= 'Z' ? 65 : 97;
    const code = startCode + ((char.charCodeAt(0) - startCode + 13) % 26);
...
function getCardId(value) {
  ...
  const index = initialDeck.indexOf(value);
  if (index !== -1) {
    return index;
  }
  return null;
